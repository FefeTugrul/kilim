...
k.region; // 'milas' — yöre de seed'den seçilir
...
> **Tam Türkçe sürüm İngilizcesinin bir faz gerisinde kalabilir.** npm sayfasında
> görünen dosya `README.md` olduğu için güncel tutulan o. Kaynak koddaki yorumlar
> ve `CONTRIBUTING.md` Türkçe.
...
| `region` | `KilimYore` | seed'den seçilir | Yöreyi sabitler. Bilinmeyen değer hata fırlatır |
...
| `region` | `KilimYore` | Seçilen yörenin kimliği |
...
```bash
npm install
npm test          # 135 test: determinizm, gramer, palet kısıtları, çeşitlilik, güvenlik
npm run typecheck
