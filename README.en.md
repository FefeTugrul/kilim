...
nothing. The pattern is computed from the string every time, so there is no file
to back up, no CDN to invalidate, and no personal data to delete on request.

Determinism is the whole point: **the seed is the record.** As long as you have
the user id, you can regenerate the pattern — which is why `Math.random` would
not be a stylistic choice here but a bug.

> The Turkish README is the primary one and is kept fully up to date; this file
> may lag behind by a phase.
